...
let modal = document.getElementById("myModal"),
    ...
    modalContent = document.getElementById('panier');

arrayData.forEach(produit => {
    let resultDisplay = document.getElementById('section');
    let div = document.createElement('div');
    div.className = 'col-3';
    div.dataset.cat = produit.category;
    div.innerHTML = `
  <img src="${produit.pics[0]}" alt="${produit.picsDescription[0]}" class="img-fluid">
  <p class="titre">${produit.product}</p>
  <p class="description">${produit.infos}</p>
  <p class="price">${produit.price}</p>
  <a href="" class="addToBasket" data-img="${produit.pics[0]}" data-price="${produit.price}" data-titre="${produit.product}" data-description="${produit.infos}" data-price="${produit.price}" data-ref="${produit.reference}">Ajouter au panier</a>
`;
...
let basket = [],
    produits = document.querySelectorAll('.addToBasket'),
    totalBasket = [],
...
produits.forEach(produit => {
    produit.addEventListener('click', function (evt) {
        evt.preventDefault();
        let index = basket.findIndex((searchArticle) => searchArticle.id === produit.dataset.ref);
        if (index !== -1) {
            incrementBasket(basket[index]);
            createBasketHTML();
            totalDisplay();
        } else {
            addToBasket(produit);
...
function incrementBasket(ligne) {
    let subTot = ligne.subtotal += parseFloat(ligne.price);
    ligne.subtotal = subTot;
    ligne.quantity++;
    totalBasket.push(parseFloat(ligne.price));
}

function decrementBasket(ligne) {
    let subTot = ligne.subtotal -= parseFloat(ligne.price);
    ligne.subtotal = subTot;
    ligne.quantity--;
    totalBasket.push(parseFloat(-ligne.price));
...
function addToBasket(produit) {
    basket.push({
        id: produit.dataset.ref,
        product: produit.dataset.titre,
        price: produit.dataset.price,
        quantity: 1,
        subtotal: parseFloat(produit.dataset.price),
    });
    totalBasket.push(parseFloat(produit.dataset.price));
    createBasketHTML(produit);
}

function createBasketHTML() {
    panier.innerHTML = "";
    basket.forEach((element, indexBasket) => {
        console.log(element)
        let rows = document.createElement('tr');
        rows.id = element.id;
        rows.dataset.index = indexBasket;
        rows.innerHTML =
            `<td>${element.id}</td>
        <td>${element.product}</td>
        <td>${element.price}</td>
        <td><button class="lessBtn" data-index="${indexBasket}" >-</button></td>
        <td>${element.quantity}</td>
        <td><button type="button" class="moreBtn" data-index="${indexBasket}">+</button></td>
        <td>${element.subtotal}</td>`
        panier.appendChild(rows);
    });
    let moreBtn = document.querySelectorAll('.moreBtn');
    moreBtn.forEach(button => {
        button.onclick = function () {
            incrementBasket(basket[this.dataset.index]);
            createBasketHTML();
            totalDisplay();
        }
    });
    let lessBtn = document.querySelectorAll('.lessBtn');
    lessBtn.forEach(button => {
        button.onclick = function () {
            if (basket[this.dataset.index].quantity <= 1) {
                totalBasket.push(parseFloat(-basket[this.dataset.index].price));
                basket.splice(this.dataset.index, 1);

            } else {
                decrementBasket(basket[this.dataset.index]);
            }
            createBasketHTML();
            totalDisplay();
        }
...
function totalDisplay() {
    ...
    let totalBasketCalc = totalBasket.reduce(reducer);
    total.textContent = "Total : " + totalBasketCalc + "€";
}
...
myBasket.onclick = function () {
    console.log(panier.childElementCount);
    if (panier.childElementCount == 0) {
        alert('Le panier est vide');
    } else if (panier.childElementCount >= 1) {
        console.log('ok');
